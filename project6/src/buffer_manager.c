...
int init_db(int buf_num, int flag, int log_num, char* log_path, char* logmsg_path){
	...
	buf_pool = (buffer_t*)malloc(sizeof(buffer_t)*buf_num);
	...
	log_buffer = (log_t*)malloc(sizeof(log_t)*buf_num);
	pthread_mutex_unlock(&log_latch);
	printf("a\n");
	log_fd = open(log_path, O_RDWR|O_CREAT|O_SYNC|O_APPEND, S_IRWXG|S_IRWXU|S_IRWXO);
//	logmsg_fd = fopen(logmsg_path, "w+");
	pthread_mutex_lock(&log_latch);
